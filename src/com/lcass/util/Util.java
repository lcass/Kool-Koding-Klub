...
public class Util {
	...
	public static Vertex2d[] rotate_xy(Vertex2d[] data, Vertex2d centre,float angle){
		Vertex2d[] out = new Vertex2d[data.length];
		for(int i = 0;i < data.length;i++){
			out[i] = rotate_xy(data[i],centre,angle);
		}
		return out;
	}
	public static Vertex2d[] rotate(ArrayList<Vertex2d> data, Vertex2d centre,float angle){
		Vertex2d[] out = new Vertex2d[data.size()];
		int  i = 0;
		for(Vertex2d data_point : data){
			out[i] = rotate(data_point,centre,angle);
			i++;
		}
		return out;
	}
	public static Vertex2d rotate_xy(Vertex2d target,Vertex2d rotation_point,float angle){
		float cos = (float)Math.cos(angle);
		float sin = (float)Math.sin(angle);
		float nx = target.x - rotation_point.x;
		float ny = target.y - rotation_point.y;
		
		return new Vertex2d(((nx * cos) - (ny * sin)) + rotation_point.x,((nx * sin) + (ny * cos))+ rotation_point.y,target.u,target.v);
	...
	public static int convert_to_two(float x){//converts a float of any value to the nearest power of 2
		int pow = (int)Math.round((Math.log(x)/Math.log(2)));
